...
// Handler for GET /api/generations/:id
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  try {
    // Get generation details using DEFAULT_USER_ID
    const generationId = validationResult.data;
    const result = await getGenerationById(generationId, DEFAULT_USER_ID, supabase);
    ...
    let status = 500;
    let message = "An error occurred while fetching the generation";
